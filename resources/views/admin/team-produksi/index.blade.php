...
@section('title', 'Team Produksi')

@section('content')
<div x-data="teamProduksiCrud()" class="space-y-6">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $teams->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $team->nama }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $team->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                                ...
                                <button @click="destroy({{ $team->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="3" class="px-5 py-12 text-center text-gray-400">Belum ada data team produksi.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-md rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Team Produksi' : 'Tambah Team Produksi'"></h3>
                <div class="mt-6 space-y-4">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Nama Team <span class="text-red-500">*</span></label>
...
function teamProduksiCrud() {
    return {
        ...
        async openEdit(id) {
            ...
            const res = await fetch(`/admin/team-produksi/${id}`, { headers: { 'Accept': 'application/json' } });
        ...
        async save() {
            ...
            const url = this.isEdit ? `/admin/team-produksi/${this.editId}` : '/admin/team-produksi';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus team?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/team-produksi/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
