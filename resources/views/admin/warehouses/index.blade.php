...
<div x-data="warehousesCrud()" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-bold text-gray-900">Warehouse</h2>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $warehouses->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $warehouse->nama }}</td>
                        <td class="px-5 py-4 text-gray-700">{{ $warehouse->no_hp ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-700 max-w-xs truncate">{{ $warehouse->alamat ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-700">{{ $warehouse->kota->nama ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $warehouse->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-green-500 text-white shadow-sm transition hover:bg-green-600">
                                ...
                                <button @click="destroy({{ $warehouse->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400">Belum ada data warehouse.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Warehouse' : 'Tambah Warehouse Baru'"></h3>
...
function warehousesCrud() {
    return {
        ...
        async openEdit(id) {
            ...
            const res = await fetch(`/admin/warehouses/${id}`, { headers: { 'Accept': 'application/json' } });
        ...
        async save() {
            ...
            const url = this.isEdit ? `/admin/warehouses/${this.editId}` : '/admin/warehouses';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus warehouse?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/warehouses/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
