            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $categories->firstItem() + $i }}</td>
                    ...
                    @empty
                    <tr><td colspan="3" class="px-5 py-12 text-center text-gray-400">Belum ada data kategori.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
function categoriesCrud() {
    return {
        ...
        async openEdit(id) {
            ...
            const res = await fetch(`/admin/categories/${id}`, { headers: { 'Accept': 'application/json' } });
        ...
        async save() {
            ...
            const url = this.isEdit ? `/admin/categories/${this.editId}` : '/admin/categories';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus kategori?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/categories/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
