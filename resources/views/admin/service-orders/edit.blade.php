...
@section('content')
<div x-data="serviceOrderEditForm()" class="space-y-6">
    ...
    <form action="{{ route('admin.service-orders.update', $serviceOrder) }}" method="POST">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                    <h3 class="text-lg font-bold text-gray-900">Informasi Servis — {{ $serviceOrder->kode_servis }}</h3>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <select x-ref="mekanikSelect" name="mekanik_id" placeholder="Ketik untuk mencari mekanik...">
                                ...
                                <option value="{{ $m->id }}" {{ $serviceOrder->mekanik_id == $m->id ? 'selected' : '' }}>{{ $m->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700">Tanggal Masuk <span class="text-red-500">*</span></label>
                            <input type="date" name="tanggal_masuk" value="{{ $serviceOrder->tanggal_masuk->toDateString() }}" class="h-10 w-full rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700">Next Service Date</label>
                            <input type="date" name="next_service_date" value="{{ $serviceOrder->next_service_date?->toDateString() }}" class="h-10 w-full rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100">
                        </div>
                    </div>
                    <div class="mt-4">
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Keluhan</label>
                        <textarea name="keluhan" rows="3" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100">{{ $serviceOrder->keluhan }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="mt-4 space-y-3">
                        <template x-for="(item, index) in jasaItems" :key="item._uid">
                            <div class="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
                                ...
                                <div class="w-40">
                                    <input type="number" :name="`jasa_items[${index}][biaya]`" x-model.number="item.biaya" class="h-10 w-full rounded-xl border border-gray-200 bg-white px-4 text-sm outline-none text-right">
                                </div>
                    ...
                    <div class="mt-4 space-y-3">
                        <template x-for="(item, index) in productItems" :key="item._uid">
                            <div class="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
                                ...
                                <div class="w-20">
                                    <input type="number" :name="`product_items[${index}][qty]`" x-model.number="item.qty" min="1" class="h-10 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm outline-none text-center">
                                </div>
                                <div class="w-36">
                                    <input type="number" :name="`product_items[${index}][harga]`" x-model.number="item.harga" class="h-10 w-full rounded-xl border border-gray-200 bg-white px-4 text-sm outline-none text-right">
                                </div>
            ...
            <div>
                <div class="sticky top-8 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-6 space-y-3">
                        <button type="button" @click="submitForm()" class="w-full rounded-xl bg-red-500 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-red-600">Update Work Order</button>
                        <a href="{{ route('admin.service-orders.show', $serviceOrder) }}" class="block w-full rounded-xl border border-gray-200 py-3 text-center text-sm font-medium text-gray-600 transition hover:bg-gray-50">Batal</a>
                    </div>
                </div>
            </div>
        </div>
...
function serviceOrderEditForm() {
    const rawJasa = @json($serviceOrder->jasaServis->map(fn($j) => ['jasa_servis_id' => (string) $j->id, 'biaya' => $j->pivot->biaya]));
    const rawProducts = @json($serviceOrder->products->map(fn($p) => ['product_id' => (string) $p->id, 'qty' => $p->pivot->qty, 'harga' => $p->pivot->harga]));

    return {
        pelanggan_id: '{{ $serviceOrder->pelanggan_id }}',
        kendaraan_id: '{{ $serviceOrder->kendaraan_id }}',
        kendaraans: @json($serviceOrder->pelanggan->kendaraans ?? []),
        jasaItems: rawJasa.map(i => ({ ...i, _uid: ++_uid })),
        productItems: rawProducts.map(i => ({ ...i, _uid: ++_uid })),
        _tomSelects: {},
        ...
        initJasaSelect(el, index) {
            ...
            this._tomSelects[key] = new TomSelect(el, {
                ...
                allowEmptyOption: true,
                items: item.jasa_servis_id ? [item.jasa_servis_id] : [],
                onChange(value) {
        ...
        async submitForm() {
            ...
            const result = await Swal.fire({ title: 'Update Work Order?', text: 'Pastikan data yang diisi sudah benar.', icon: 'question', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, update!', cancelButtonText: 'Batal' });
