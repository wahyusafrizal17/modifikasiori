...
<div x-data="kotasCrud()" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-bold text-gray-900">Kota</h2>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500">Nama Kota</th>
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500 w-28"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($kotas as $i => $kota)
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $kotas->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $kota->nama }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $kota->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-green-500 text-white shadow-sm transition hover:bg-green-600">
                                ...
                                <button @click="destroy({{ $kota->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="3" class="px-5 py-12 text-center text-gray-400">Belum ada data kota.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        @if($kotas->hasPages())
        <div class="mt-5">{{ $kotas->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-md rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Kota' : 'Tambah Kota Baru'"></h3>
                <div class="mt-6 space-y-4">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Nama Kota</label>
...
function kotasCrud() {
    return {
        ...
        async openEdit(id) {
            ...
            const res = await fetch(`/admin/kotas/${id}`, { headers: { 'Accept': 'application/json' } });
        ...
        async save() {
            ...
            const url = this.isEdit ? `/admin/kotas/${this.editId}` : '/admin/kotas';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus kota?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/kotas/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
