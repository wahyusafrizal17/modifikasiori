...
<div x-data="pelangganCrud()" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-bold text-gray-900">Pelanggan</h2>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        ...
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $pelanggan->nama }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $pelanggan->no_hp ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $pelanggan->kota->nama ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $pelanggan->kendaraans_count }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.pelanggans.show', $pelanggan) }}" class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500 text-white shadow-sm transition hover:bg-blue-600" title="Detail">
                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <button @click="openEdit({{ $pelanggan->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-green-500 text-white shadow-sm transition hover:bg-green-600">
                                ...
                                <button @click="destroy({{ $pelanggan->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400">Belum ada data pelanggan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Pelanggan' : 'Tambah Pelanggan Baru'"></h3>
                <div class="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea x-model="form.alamat" rows="2" class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100"></textarea>
                    </div>
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Kota</label>
                        <select x-model="form.kota_id" class="h-10 w-full rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100">
...
function pelangganCrud() {
    return {
        ...
        openCreate() {
            ...
            this.form = { nama: '', no_hp: '', alamat: '', kota_id: '' };
            this.showModal = true;
        },
        async openEdit(id) {
            this.isEdit = true; this.editId = id; this.errors = {};
            const res = await fetch(`/admin/pelanggans/${id}`, { headers: { 'Accept': 'application/json' } });
            const data = await res.json();
            this.form = { nama: data.nama, no_hp: data.no_hp || '', alamat: data.alamat || '', kota_id: data.kota_id || '' };
            this.showModal = true;
        },
        async save() {
            this.loading = true; this.errors = {};
            const url = this.isEdit ? `/admin/pelanggans/${this.editId}` : '/admin/pelanggans';
        ...
        async destroy(id) {
            ...
            await fetch(`/admin/pelanggans/${id}`, {
