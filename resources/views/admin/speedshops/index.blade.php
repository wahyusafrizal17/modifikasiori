...
@section('title', 'Speedshop')

@section('content')
<div x-data="speedshopsCrud()" class="space-y-6">
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('admin.dashboard') }}" class="transition hover:text-gray-700">Home</a>
        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        <span class="font-medium text-gray-900">Speedshop</span>
    </nav>
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-bold text-gray-900">Speedshop</h2>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500">No. HP/Telp.</th>
                        ...
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500">Warehouse Terkait</th>
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500 w-28"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($speedshops as $i => $speedshop)
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $speedshops->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $speedshop->nama }}</td>
                        <td class="px-5 py-4 text-gray-700">{{ $speedshop->no_hp ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-700 max-w-xs truncate">{{ $speedshop->alamat ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-700">{{ $speedshop->kota->nama ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-700">{{ $speedshop->warehouse->nama ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $speedshop->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                                ...
                                <button @click="destroy({{ $speedshop->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="7" class="px-5 py-12 text-center text-gray-400">Belum ada data speedshop.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        @if($speedshops->hasPages())
        <div class="mt-5">{{ $speedshops->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Speedshop' : 'Tambah Speedshop Baru'"></h3>
                <div class="mt-6 space-y-4">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">No. HP/Telp.</label>
                        <input type="text" x-model="form.no_hp" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20" :class="errors.no_hp && 'border-red-400'">
                    ...
                    <div>
                        ...
                        <textarea x-model="form.alamat" rows="3" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20" :class="errors.alamat && 'border-red-400'"></textarea>
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Warehouse Terkait</label>
                        <select x-model="form.warehouse_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20" :class="errors.warehouse_id && 'border-red-400'">
                            <option value="">Pilih Warehouse (opsional)</option>
                            @foreach($warehouses as $wh)
                                <option value="{{ $wh->id }}">{{ $wh->nama }}</option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-500">Warehouse yang mensupply / terkait untuk produk & mutasi.</p>
...
@push('scripts')
<script>
function speedshopsCrud() {
    return {
        showModal: false, isEdit: false, editId: null, loading: false, errors: {},
        form: { nama: '', no_hp: '', alamat: '', kota_id: '', warehouse_id: '' },
        openCreate() { this.isEdit = false; this.editId = null; this.errors = {}; this.form = { nama: '', no_hp: '', alamat: '', kota_id: '', warehouse_id: '' }; this.showModal = true; },
        async openEdit(id) {
            this.isEdit = true; this.editId = id; this.errors = {}; this.loading = true; this.showModal = true;
            const res = await fetch(`/admin/speedshops/${id}`, { headers: { 'Accept': 'application/json' } });
            const data = await res.json();
            this.form = { nama: data.nama, no_hp: data.no_hp || '', alamat: data.alamat || '', kota_id: data.kota_id ? String(data.kota_id) : '', warehouse_id: data.warehouse_id ? String(data.warehouse_id) : '' };
            this.loading = false;
        },
        async save() {
            this.loading = true; this.errors = {};
            const url = this.isEdit ? `/admin/speedshops/${this.editId}` : '/admin/speedshops';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus speedshop?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            const res = await fetch(`/admin/speedshops/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            const data = await res.json();
            if (!res.ok && data.message) { Swal.fire({ icon: 'error', title: 'Gagal', text: data.message }); return; }
            window.location.reload();
        }
    }
}
