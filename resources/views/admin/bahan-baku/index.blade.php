...
@section('content')
<div x-data="bahanBakuCrud()" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div class="mt-6 overflow-x-auto rounded-xl border border-gray-100">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($bahanBakus as $i => $bb)
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $bahanBakus->firstItem() + $i }}</td>
                        ...
                        <td class="px-5 py-4 text-right font-semibold text-gray-900">{{ number_format($bb->stok) }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $bb->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                                ...
                                <button @click="destroy({{ $bb->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="7" class="px-5 py-12 text-center text-gray-400">Belum ada data bahan baku.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-5">{{ $bahanBakus->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Bahan Baku' : 'Tambah Bahan Baku'"></h3>
...
function bahanBakuCrud() {
    return {
        ...
        async openEdit(id) {
            ...
            const res = await fetch(`/admin/bahan-baku/${id}`, { headers: { 'Accept': 'application/json' } });
        ...
        async save() {
            ...
            const url = this.isEdit ? `/admin/bahan-baku/${this.editId}` : '/admin/bahan-baku';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus bahan baku?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/bahan-baku/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
