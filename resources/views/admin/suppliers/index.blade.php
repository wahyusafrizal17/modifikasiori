            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500">Nama Supplier</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $suppliers->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $supplier->nama }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $supplier->no_hp ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $supplier->kota->nama ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-600 max-w-xs truncate">{{ $supplier->alamat ?? '-' }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button @click="openEdit({{ $supplier->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                                ...
                                <button @click="destroy({{ $supplier->id }})" class="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600">
                    ...
                    @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400">Belum ada data supplier.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" @keydown.escape.window="showModal = false">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="showModal" x-transition class="relative w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl" @click.stop>
                <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Supplier' : 'Tambah Supplier Baru'"></h3>
                <div class="mt-6 space-y-4">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700">Nama Supplier <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" x-model="form.no_hp" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700">Kota</label>
                            <select x-model="form.kota_id" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20">
                    ...
                    <div>
                        ...
                        <textarea x-model="form.alamat" rows="2" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-400/20"></textarea>
                    </div>
                </div>
...
function suppliersCrud() {
    return {
        ...
        form: { nama: '', no_hp: '', alamat: '', kota_id: '' },
        openCreate() { this.isEdit = false; this.editId = null; this.errors = {}; this.form = { nama: '', no_hp: '', alamat: '', kota_id: '' }; this.showModal = true; },
        async openEdit(id) {
            this.isEdit = true; this.editId = id; this.errors = {}; this.loading = true; this.showModal = true;
            const res = await fetch(`/admin/suppliers/${id}`, { headers: { 'Accept': 'application/json' } });
            const data = await res.json();
            this.form = { nama: data.nama, no_hp: data.no_hp || '', alamat: data.alamat || '', kota_id: data.kota_id ? String(data.kota_id) : '' };
            this.loading = false;
        },
        async save() {
            this.loading = true; this.errors = {};
            const url = this.isEdit ? `/admin/suppliers/${this.editId}` : '/admin/suppliers';
        ...
        async destroy(id) {
            const result = await Swal.fire({ title: 'Hapus supplier?', text: 'Data yang dihapus tidak dapat dikembalikan.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' });
            if (!result.isConfirmed) return;
            await fetch(`/admin/suppliers/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
            window.location.reload();
        }
    }
}
