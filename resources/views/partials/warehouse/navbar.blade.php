        ...
        <div class="relative" x-data="notifDropdown()" x-init="fetchNotifications()">
            ...
            <div x-show="open" @click.away="open = false" x-cloak
                ...
                <div class="flex items-center justify-between border-b border-gray-100 px-4 py-3">
                    ...
                    <span x-show="unreadCount > 0" class="inline-flex items-center rounded-full bg-red-50 px-2 py-0.5 text-[10px] font-semibold text-red-600" x-text="unreadCount + ' Baru'"></span>
                </div>
                    ...
                    <template x-for="notif in notifications" :key="notif.id">
                        <a :href="notif.link || '#'" @click="markRead(notif)" class="flex gap-3 border-b border-gray-50 px-4 py-3 transition hover:bg-gray-50" :class="!notif.read_at && 'bg-red-50/50'">
                            <div class="flex-shrink-0 mt-0.5">
                                <div class="flex h-8 w-8 items-center justify-center rounded-full" :class="['stock_in_approved','warehouse_transaksi_approved'].includes(notif.type) ? 'bg-green-100' : (['stock_in_rejected','warehouse_transaksi_rejected'].includes(notif.type) ? 'bg-red-100' : (notif.type === 'mutasi_produk' ? 'bg-blue-100' : 'bg-yellow-100'))">
                                    <svg x-show="['stock_in_approved','warehouse_transaksi_approved'].includes(notif.type)" class="h-4 w-4 text-green-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                    <svg x-show="['stock_in_rejected','warehouse_transaksi_rejected'].includes(notif.type)" class="h-4 w-4 text-red-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    ...
                                    <svg x-show="!['stock_in_approved','stock_in_rejected','mutasi_produk','warehouse_transaksi_approved','warehouse_transaksi_rejected'].includes(notif.type)" class="h-4 w-4 text-yellow-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                </div>
                            </div>
