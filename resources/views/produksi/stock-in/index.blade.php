...
@section('title', 'Stock IN - Bahan Baku & Kemasan')
...
<div class="space-y-6">
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span class="font-medium text-gray-900">Stock IN</span>
    </nav>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Total Stock IN</p>
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-900">Stock IN Bahan Baku & Kemasan</h2>
                <p class="mt-1 text-sm text-gray-500">Input stok masuk bahan baku dan kemasan produksi</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('produksi.stock-in.create') }}" class="inline-flex h-10 items-center gap-2 rounded-xl bg-red-500 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-red-600 active:scale-[0.98]">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="transition hover:bg-gray-50">
                        <td class="px-5 py-4 text-gray-500">{{ $stockIns->firstItem() + $i }}</td>
                        <td class="px-5 py-4 font-medium text-gray-900">{{ $si->kode }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $si->created_at->format('d M Y H:i') }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $si->user->name }}</td>
                        <td class="px-5 py-4 text-center text-gray-600">{{ $si->items->count() }}</td>
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4">
                            <a href="{{ route('produksi.stock-in.show', $si) }}" class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-500 text-white shadow-sm transition hover:bg-gray-600" title="Detail">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="px-5 py-12 text-center text-gray-400">Belum ada data stock in.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-5">{{ $stockIns->links() }}</div>
        @endif
    </div>
</div>
@endsection
