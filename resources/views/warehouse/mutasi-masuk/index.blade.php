...
@section('title', $config['pageTitle'] ?? 'Mutasi Masuk')
...
@php
    $config = $config ?? [
        ...
        'showSumberFilter' => false,
        'showCreateMutasiSpeedshop' => false,
    ];
@endphp
<div class="space-y-8">
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('warehouse.dashboard') }}" class="transition hover:text-gray-700">Home</a>
        <svg class="h-3.5 w-3.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        <span class="font-medium text-gray-900">{{ $config['breadcrumb'] }}</span>
    </nav>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <form method="GET" action="{{ route($config['indexRoute']) }}" class="flex flex-wrap items-center gap-2">
                    @if(!empty($config['showSumberFilter']))
                    <select name="sumber" onchange="this.form.submit()" class="h-11 rounded-xl border border-gray-200 bg-gray-50 px-3 text-sm text-gray-700 transition focus:border-red-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-400/20">
                        <option value="">Semua Sumber</option>
                        <option value="produksi" {{ request('sumber') === 'produksi' ? 'selected' : '' }}>Dari Produksi</option>
                        <option value="warehouse" {{ request('sumber') === 'warehouse' ? 'selected' : '' }}>Ke Speedshop</option>
                    </select>
                    @endif
                ...
                <a href="{{ route('warehouse.transaksi-speedshop.create') }}" class="ml-auto inline-flex h-11 items-center gap-2 rounded-xl bg-red-500 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-red-600">
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        @endif
                        <th class="px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-gray-500">Dari Produksi</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        $detailUrl = $config['indexRoute'] === 'warehouse.transaksi-speedshop'
                            ? route('warehouse.transaksi-speedshop.show', $m)
                            : route('warehouse.mutasi-masuk.show', $m);
                        $isVerified = $m->status === 'diterima';
                    @endphp
                    <tr class="transition hover:bg-gray-50">
                        ...
                        <td class="px-5 py-4">
                            @if($m->isFromWarehouse())
                                <span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-semibold text-blue-700">Ke Speedshop</span>
                            @else
                                <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-700">Dari Produksi</span>
                            @endif
                        </td>
                        @endif
                        <td class="px-5 py-4 text-gray-700">{{ $m->user->name ?? '-' }}</td>
                        ...
                        <td class="px-5 py-4">
                            <a href="{{ $detailUrl }}" class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-500 text-white shadow-sm transition hover:bg-gray-600" title="Lihat Detail">
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ !empty($config['showSumberFilter']) ? 10 : 9 }}" class="px-5 py-12 text-center text-gray-400">
