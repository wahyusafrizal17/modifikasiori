...
@section('content')
@php
    $fromTransaksiSpeedshop = $fromTransaksiSpeedshop ?? false;
    $listUrl = $fromTransaksiSpeedshop ? route('warehouse.transaksi-speedshop') : route('warehouse.mutasi-masuk.index');
    $listLabel = $fromTransaksiSpeedshop ? 'Transaksi Speedshop (Mutasi)' : 'Mutasi Masuk';
@endphp
<div class="space-y-6">
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('warehouse.dashboard') }}" class="transition hover:text-gray-700">Home</a>
        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        <a href="{{ $listUrl }}" class="transition hover:text-gray-700">{{ $listLabel }}</a>
        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                ...
                <p class="mt-1 text-sm text-gray-500">{{ $mutasi->isFromWarehouse() ? 'Mutasi dari Warehouse ke Speedshop' : 'Produk dari Produksi menuju Warehouse Anda' }}</p>
        ...
        <div class="space-y-6">
            <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                ...
                @if($mutasi->isDikirim())
                @if($mutasi->isFromWarehouse())
                <div class="mt-5 rounded-lg border border-blue-200 bg-blue-50 p-3">
                    <p class="text-xs font-semibold text-blue-800">Dikirim ke Speedshop</p>
                    <p class="mt-1 text-xs text-blue-700">Mutasi ini menunggu verifikasi penerimaan dari tim Speedshop.</p>
                </div>
                ...
                <div class="mt-5 space-y-3">
                    ...
                    <p class="text-center text-xs text-amber-700">Klik untuk memverifikasi. Stok produk akan ditambahkan ke warehouse.</p>
                </div>
                @else
                <div class="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-3">
                    ...
                    <p class="mt-1 text-xs text-amber-700">Produk belum masuk stok Warehouse. Hubungi Manager Warehouse untuk konfirmasi penerimaan.</p>
                </div>
                @endif
                @endif

                <div class="mt-5">
                    <a href="{{ $listUrl }}" class="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 px-5 py-3 text-sm font-medium text-gray-600 transition hover:bg-gray-50">
...
@if($mutasi->isDikirim() && $mutasi->isFromProduksi() && (auth()->user()->isManager() || auth()->user()->isAdmin()))
@push('scripts')
<script>
const verifyUrl = '{{ (request()->routeIs("warehouse.transaksi-speedshop.show") ? route("warehouse.transaksi-speedshop.verify", $mutasi) : route("warehouse.mutasi-masuk.verify", $mutasi)) }}';

async function verifyMutasi() {
    const result = await Swal.fire({
        title: 'Verifikasi Mutasi?',
        text: 'Stok produk akan ditambahkan ke warehouse. Tindakan ini tidak dapat dibatalkan.',
        icon: 'question',
